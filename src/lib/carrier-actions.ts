...
// //   userId: passengerId,
// //   title: "عرض جديد وصلك! 🚗",
// //   body: "وصلك عرض جديد من ناقل، راجع التفاصيل الآن.",
// //   data: { type: "new_offer" },
// // }); // 'use client';
    ...
    try {
      ...
      if (tripSnap.exists()) {
        ...
        if (passengerId) {
          ...
          // [PUSH]: إشعار Push للمسافر
          await sendPush({
            userId: passengerId,
            title: "عرض جديد وصلك! 🚗",
            body: `الناقل "${carrierName || "ناقل"}" أرسل عرضاً بسعر ${offerData.price} ${offerData.currency}`,
            data: { type: "new_offer", tripId: passengerIntentId },
          });
        }
      }
    } catch (notifError) {
