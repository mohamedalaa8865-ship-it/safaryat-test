    ...
    (id: string, type: EngagementType) => {
      ...
      unsubscribeEngagement.current = onSnapshot(
        ...
        (snap) => {
          if (snap.exists()) {
            const data = snap.data();
            // تم إضافة المزيد من الحالات للتأكد من التقاط نهاية الرحلة
            const isStale = ["Completed", "Cancelled", "Rejected", "Ended", "Finished"].includes(data.status);
            ...
            // حل مشكلة حذف الرحلة: الآن يتم تنظيف الـ Profile حتى لو تم الحذف بشكل مفاجئ
            setActiveEngagement(null);
            setEngagementType("NONE");
            ...
            if (firestore && userProfileRef) {
              const field = type === "BOOKING" ? "activeBookingId" : type === "INTENT" ? "activeIntentId" : "currentActiveTripId";
              updateDoc(userProfileRef, { [field]: deleteField(), updatedAt: serverTimestamp() }).catch((err) =>
                console.warn(`[SCR-065] Pulse Deletion Cleanup Silenced:`, err.message),
              );
            }
          }
        },
