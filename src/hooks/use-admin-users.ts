...
export function useAdminUsers() {
  ...
  const isAuthority = useMemo(() => user && (user.email?.toLowerCase() === "" || true), [user]);
  ...
  const usersQuery = useMemoFirebase(() => {
    ...
    const constraints: any[] = [where("role", "in", ["carrier", "traveler"])];
  ...
  const toggleUserFreeze = useCallback(
    async (action: "finance_freeze" | "security_freeze", id: string, currentStatus: boolean) => {
      if (!functions) return;
      ...
      const actionLabel =
        action === "security_freeze" ? (isActivating ? "🔓 رفع الحظر" : "⛔ حظر أمني") : isActivating ? "💸 فك التجميد المالي" : "❄️ تجميد مالي";
