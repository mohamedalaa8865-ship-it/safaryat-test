...
//   // [FIX-SEATS]: بنحتفظ بعدد المقاعد المختارة من الـ sheet
//   const [selectedSeatCount, setSelectedSeatCount] = useState(1);

//   const handleSheetConfirm = useCallback((seatCount: number) => {
...
//         email, tripId: trip.id, carrierId: trip.carrierId, carrierTripId: trip.id, seatCount: selectedSeatCount, status: 'pending',
...
export function ScheduledTripCard({
  ...
  const handleEmailConfirmed = useCallback(async (email: string) => {
    ...
    try {
      const tokenDoc = await addDoc(collection(firestore, 'booking_tokens'), {
        email, tripId: trip.id, carrierId: trip.carrierId, carrierTripId: null, seatCount: selectedSeatCount, status: 'pending',
        createdAt: serverTimestamp(), expiresAt: new Date(Date.now() + 30 * 60 * 1000),
      });
      tokenDocId = tokenDoc.id;
      const confirmUrl = `${origin}/${locale}/confirm-booking?token=${tokenDocId}`;
