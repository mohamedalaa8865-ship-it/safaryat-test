...
//                   const Icon = item.icon;
//                   return (
//                     // ✅ FIX: key على الـ Link مباشرة بدون Fragment wrapper
//                     <Link
//                       key={item.href}
//                       href={item.href}
//                       className={`flex items-center gap-3 text-lg font-medium transition-colors hover:text-primary ${pathname === item.href ? "text-primary" : "text-muted-foreground"
...
//                       {item.label}
//                       {item.count != null && item.count > 0 ? (
//                         <span className="bg-destructive text-destructive-foreground text-[10px] rounded-full h-4 w-4 flex items-center justify-center ml-auto">
//                           {item.count}
//                         </span>
//                       ) : null}
//                     </Link>
...
export function AppLayout({ children, profile, user, isEngaged = false, engagementType = 'NONE' }: AppLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-y-auto">
      <header className="app-navbar fixed top-0 left-0 right-0 z-50 w-full bg-[#bead77] border-b backdrop-blur">
        <div className="container mx-auto pt-2 flex h-16 items-center justify-between px-4">
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="md:hidden">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="app-navbar-link transition-colors"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" className="relative h-9 w-9 rounded-full border border-black  bg-black hover:bg-[#2A1E1A]">
                      <Avatar className="h-9 w-9 border-2 border-primary/10">
                        <AvatarImage src={user.photoURL || ""} alt="User" />
                        <AvatarFallback className="bg-primary/5 text-primary"><User className="h-4 w-4 " /></AvatarFallback>
                      </Avatar>
