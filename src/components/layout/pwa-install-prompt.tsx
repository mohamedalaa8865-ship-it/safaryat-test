...
// // // ✅ Production mode — غيّره لـ true بس لو بتعمل debug
...
// //     // STEP 2: هل الـ user سبق رفض؟
// //     try {
// //       const dismissed = localStorage.getItem('pwa_prompt_dismissed');
// //       if (dismissed) {
// //         const age = Date.now() - parseInt(dismissed);
// //         if (age < 7 * 24 * 60 * 60 * 1000) return; // 7 أيام
// //       }
// //     } catch (_) {}
...
// //     if (isAppleDevice) {
// //       // iOS: عرض instructions بعد 8 ثواني
// //       const timer = setTimeout(() => setShowPrompt(true), 8000);
// //       return () => clearTimeout(timer);
// //     } else {
// //       // Android: استخدم window.__pwaPrompt المحفوظ من inline script في layout
// //       const saved = (window as any).__pwaPrompt;
// //       if (saved) {
// //         setDeferredPrompt(saved);
...
// //       aria-modal="true"
// //       aria-label={t('title')}
// //       className="fixed bottom-20 left-4 right-4 bg-card border-2 border-primary/30 p-4 rounded-2xl shadow-2xl z-[9998] animate-in slide-in-from-bottom-10 duration-500"
// //     >
// //       <div className="flex items-start gap-3">
// //         <div
// //           aria-hidden="true"
// //           className="h-11 w-11 bg-primary/10 rounded-xl flex items-center justify-center shrink-0 mt-0.5"
// //         >
// //           <Sparkles className="h-5 w-5 text-primary animate-pulse" />
// //         </div>
...
// //           {isIOS ? (
// //             // ✅ FIX: كان بيعرض androidDesc على iOS — صُلّح
// //             <div className="text-[11px] text-muted-foreground leading-relaxed">
// //               <p className="mb-1.5">{t('iosDesc')}</p>
// //               <ol className="flex flex-col gap-1" aria-label={t('iosStepsLabel')}>
// //                 <li className="flex items-center gap-1.5">
// //                   <span
// //                     aria-hidden="true"
// //                     className="w-4 h-4 rounded-full bg-primary/20 text-primary text-[9px] flex items-center justify-center font-bold shrink-0"
// //                   >
// //                     ١
// //                   </span>
// //                   <span>{t('step1')}</span>
...
// const DEBUG_MODE = true; // ← مؤقتاً true عشان نشوف المشكلة
...
//   const isHomePage = /^\/(ar|en)?\/?$/.test(pathname);
//   const isLoginPage = /^\/(ar|en)?\/email-login?role=(\d+)?$/.test(pathname);

//   useEffect(() => {
//     // console.log('[PWA] pathname:', pathname);
//     // console.log('[PWA] isHomePage:', isHomePage);

//     if (!isHomePage && !isLoginPage) {
//       // console.log('[PWA] ❌ مش في الهوم — مش هيظهر');
//       return;
//     }
//     if (typeof window === 'undefined') return;
...
//     console.log('[PWA] isStandalone (already installed):', isAppMode);
//     setIsStandalone(isAppMode);
//     if (isAppMode) {
//       // console.log('[PWA] ❌ التطبيق متثبت أصلاً — مش هيظهر');
//       return;
//     }
...
//     // console.log('[PWA] isIOS:', isAppleDevice);
//     setIsIOS(isAppleDevice);
...
//           // console.log('[PWA] ❌ المستخدم رفضه من أقل من 7 أيام');
//           return;
//         }
//       }
//     }
...
//     if (isAppleDevice) {
//       console.log('[PWA] ✅ iOS — هيظهر بعد ثانية');
//       setTimeout(() => setShowPrompt(true), 1000);
//     } else {
//       const saved = (window as any).__pwaPrompt;
//       // console.log('[PWA] __pwaPrompt موجود:', !!saved);

//       if (saved) {
//         setDeferredPrompt(saved);
//         // console.log('[PWA] ✅ Android مع prompt — هيظهر بعد ثانية');
//         setTimeout(() => setShowPrompt(true), 1000);
//       } else {
//         // DEBUG: نعرضه حتى لو مفيش native prompt
//         // console.log('[PWA] ✅ DEBUG — هيظهر بعد ثانية بدون native prompt');
//         setTimeout(() => setShowPrompt(true), 1000);
//       }
...
//         (window as any).__pwaPrompt = e;
//         setDeferredPrompt(e);
//         // console.log('[PWA] ✅ beforeinstallprompt event جه!');
...
//   if (!isHomePage || isStandalone || !showPrompt) return null;
...
export function PwaInstallPrompt() {
  ...
  const isHomePage = /^\/(ar|en)?\/?$/.test(pathname);
  // ✅ تعديل الـ Regex: usePathname لا يحتوي على query parameters
  const isLoginPage = /^\/(ar|en)?\/email-login\/?$/.test(pathname);
  ...
  }, [isHomePage, isLoginPage]); // ✅ إضافة isLoginPage للمصفوفة
  ...
  // ✅ التعديل الأهم: السماح للمكون بالظهور إذا كانت الصفحة هي الهوم أو صفحة اللوج إن
  if ((!isHomePage && !isLoginPage) || isStandalone || !showPrompt) return null;
