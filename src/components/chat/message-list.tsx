...
const safeFormat = (timestamp: any, clientTimestamp?: string) => {
  try {
    // لو timestamp موجود (النت شغال أو رجع) استخدمه
    if (timestamp) {
      const date = timestamp?.toDate ? timestamp.toDate() : new Date(timestamp);
      if (!isNaN(date.getTime())) return format(date, 'p', { locale: arSA });
    }
    // fallback: استخدم clientTimestamp المحفوظ وقت الإرسال
    if (clientTimestamp) {
      ...
      {messages.map((message, index) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-[11px] text-muted-foreground mt-0.5 px-1">
              {safeFormat(message.timestamp, (message as any).clientTimestamp)}
            </span>
          </div>
        );
      })}
