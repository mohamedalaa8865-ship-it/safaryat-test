...
export function EmailConfirmDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      // ضفنا await هنا عشان الكود يقف ويستنى العملية تخلص
      await onConfirm(email);
    } catch (error) {
      console.error("Error during confirmation:", error);
    } finally {
      // الخطوة دي هتتنفذ بعد ما الـ onConfirm تخلص، سواء بنجاح أو بفشل
      setLoading(false);
    }
  };
  const handleClose = () => {
          ...
          <div className="bg-muted/50 rounded-lg p-3 text-sm space-y-1.5 border">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {offer?.depositPercentage ?? trip.depositPercentage ?? 0}%
                {(offer?.price && offer?.depositPercentage) ? (
