...
// // // //   isOpen: boolean;
// // // //   onClose: () => void;
// // // //   onConfirm: (seatCount: number) => void;  // [FIX-SEATS]: بنمرر عدد المقاعد
// // // //   trip?: Trip;
// // // //   countryCode?: string;
// // // //   confirmedEmail?: string;
// // // // }
...
// // // //   const { rule, loading } = useCountryPricing(countryCode);
// // // //   const [isConfirming, setIsConfirming] = useState(false);
// // // //   // [FIX-SEATS]: state لعدد المقاعد — الـ default 1
// // // //   const [seatCount, setSeatCount] = useState(1);

// // // //   const maxSeats = trip?.availableSeats && trip.availableSeats > 0 ? Math.min(trip.availableSeats, 10) : 10;
...
// // // //     setIsConfirming(true);
// // // //     try {
// // // //       await onConfirm(seatCount);  // [FIX-SEATS]: بنمرر عدد المقاعد
// // // //     } finally {
// // // //       setIsConfirming(false);
// // // //     }
// // // //   }, [onConfirm, seatCount]);
...
// // // //   return (
// // // //     <Sheet open={isOpen} onOpenChange={onClose}>
// // // //       <SheetContent side="bottom" className="rounded-t-xl px-6 pb-8 pt-9 h-auto max-h-[90vh]">
...
// // //   onConfirm: (seatCount: number) => void;  // [FIX-SEATS]: بنمرر عدد المقاعد
// // //   trip?: Trip;
// // //   offer?: any; // بيانات العرض (السعر، اسم الناقل، العربون)
// // //   countryCode?: string;
// // //   confirmedEmail?: string;
// // // }
...
// // //  */
// // // export function BookingSummarySheet({
// // //   isOpen, onClose, onConfirm, trip, offer, countryCode = 'JO', confirmedEmail = '',
// // // }: BookingSummaryProps) {
...
// // //               <div className="flex justify-between">
// // //                 <span>{t('carrier')}</span>
// // //                 <span>{offer?.carrierName || trip?.carrierName || '--'}</span>
// // //               </div>
// // //               <div className="flex justify-between">
// // //                 <span>{t('ticketPrice')}</span>
// // //                 <span className="font-semibold">
// // //                   {offer?.price ?? trip?.price ?? '--'} {offer?.currency ?? trip?.currency ?? ''}
// // //                 </span>
// // //               </div>
// // //               {(offer?.depositPercentage ?? trip?.depositPercentage) ? (
// // //                 <div className="flex justify-between text-amber-400">
// // //                   <span>العربون ({offer?.depositPercentage ?? trip?.depositPercentage}%)</span>
// // //                   <span className="font-semibold font-mono">
// // //                     {(() => {
// // //                       const price = offer?.price ?? trip?.price ?? 0;
// // //                       const pct = offer?.depositPercentage ?? trip?.depositPercentage ?? 0;
// // //                       return ((price * pct) / 100).toFixed(2);
// // //                     })()} {offer?.currency ?? trip?.currency ?? ''}
// // //                   </span>
// // //                 </div>
// // //               ) : null}
// // //             </div>
...
// //                 <div className="flex justify-between items-center mt-1 bg-orange-950/40 border border-orange-700/50 rounded-md px-3 py-2">
...
//     // حساب العربون من بيانات الرحلة
//     const tripPrice = offer?.price ?? trip?.price ?? 0;
//     const depositPct = offer?.depositPercentage ?? trip?.depositPercentage ?? 0;
//     const depositAmount = depositPct > 0 ? (tripPrice * depositPct) / 100 : 0;
//     const tripCurrency = offer?.currency ?? trip?.currency ?? currency;

//     return { platformFee, discount, totalDue, currency, isFreePeriod, depositAmount, tripCurrency };
//   }, [rule, offer, trip]);
...
//                 <div className="flex justify-between items-center font-extrabold space-x-1 text-white">
//                   <span>{t('serviceFee')}</span>
//                   <span className="font-mono">
//                     {financials.platformFee.toFixed(2)} {financials.currency}
//                   </span>
...
//                 <div className="flex justify-between items-center space-x-1 text-green-800 font-bold bg-green-50 p-2 rounded">
//                   <span className="flex items-center gap-1">
//                     {financials.isFreePeriod ? <Sparkles className="w-4 h-4" /> : <Ticket className="w-4 h-4" />}
//                     {financials.isFreePeriod ? t('freeLaunch') : t('promoDiscount')}
//                   </span>
...
//                     ? <>{financials.depositAmount.toFixed(2)} <span className="text-sm">{financials.tripCurrency}</span></>
//                     : <>{financials.totalDue.toFixed(2)} <span className="text-sm">{financials.currency}</span></>
//                   }
//                 </span>
//               </div>
...
//             <div className="bg-gray-900 p-4 rounded-lg border border-[#AE9E6D] space-y-2 text-sm">
//               <div className="flex justify-between">
//                 <span>{t('route')}</span>
//                 <span className="font-semibold">{trip?.origin || '--'} ➡ {trip?.destination || '--'}</span>
//               </div>
...
//               ) : null}
//               {(trip?.excessWeightFee != null && trip.excessWeightFee > 0) && (
//                 <div className="flex justify-between items-center mt-1 bg-orange-950/40 border border-[#AE9E6D] rounded-md px-3 py-2">
//                   <span className="flex items-center gap-1.5 text-orange-400 text-xs font-bold">
//                     ⚖️ رسوم الوزن الزائد
//                   </span>
//                   <span className="text-orange-300 font-mono text-xs font-bold">
...
//                 </div>
//               )}
//               {(offer?.conditions || trip?.conditions) && (
//                 <div className="mt-2 bg-yellow-950/40 border border-[#AE9E6D] rounded-md px-3 py-2 space-y-1 flex justify-between items-center">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-900 p-4 rounded-lg border border-[#AE9E6D] space-y-2 text-sm">
              ...
              {(offer?.conditions || trip?.conditions) && (
                <div className="mt-2 bg-yellow-950/40 border border-[#AE9E6D] rounded-md px-3 py-2 space-y-1 flex justify-between items-center">
                  <span className="flex items-center gap-1.5 text-yellow-400 text-sm font-black">
                    ⚠️ شروط الناقل
                  </span>
                  <p className="text-yellow-200 text-sm font-semibold whitespace-pre-wrap leading-relaxed">
                    {offer?.conditions || trip?.conditions}
                  </p>
                </div>
              )}
              ...
              <div className="flex justify-between items-center text-lg font-bold text-white">
                ...
                <span className="font-mono text-2xl">
                  {financials.depositAmount > 0 ? (
                    ...
                    <span className="text-green-400 text-xl font-sans">الدفع عند الالتقاء</span>
                  ) : (
                    <>{financials.totalDue.toFixed(2)} <span className="text-sm">{financials.currency}</span></>
                  )}
                </span>
            ...
            <SheetFooter>
              <Button
                onClick={handleConfirm}
                disabled={isConfirming}
                className="w-full h-12 text-lg font-bold bg-[#307380] text-white hover:bg-[#307380]"
              >
                {isConfirming ? (
                  <><Loader2 className="ml-2 h-5 w-5 animate-spin" />{t('confirming')}</>
                ) : (
