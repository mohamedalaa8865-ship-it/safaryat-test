...
const ConfirmedTripWrapper = ({ booking, onRate, onCancel, onPrivateChat, onGroupChat, onClear }: any) => {
    ...
    // ✅ [FIX]: استخدم carrierTripId أولاً (رحلة الناقل الفعلية) وإلا tripId كـ fallback
    const effectiveTripId = booking.carrierTripId || booking.tripId;
    const tripRef = useMemoFirebase(() => firestore ? doc(firestore, 'trips', effectiveTripId) : null, [firestore, effectiveTripId]);
    const carrierRef = useMemoFirebase(() => firestore ? doc(firestore, 'users', booking.carrierId) : null, [firestore, booking.carrierId]);
