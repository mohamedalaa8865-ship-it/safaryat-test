...
import {
  Home, CircleUser, ShieldCheck, LogOut,
  Banknote, Shield, Zap, MessageSquare, Globe, Code,
  History, Terminal, Coins, UserCheck
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';
...
/**
 * @layout AdminLayout
 * [PERF-FIX-3]: Removed mounted/useEffect guard.
 * Admin layout is only loaded after login — no hydration flash risk here.
 * The real guards are isAdminLoading + isProfileLoading checks below.
 */
export default function AdminLayout({ children }: { children: ReactNode }) {
  const { isAdmin, isLoading: isAdminLoading } = useAdmin();
  const { profile, checkPermission, isLoading: isProfileLoading, isMaster } = useUserProfile();
  ...
  if (!(isOwner || isAdmin)) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center gap-4 p-8 text-center bg-background">
        <ShieldCheck className="h-12 w-12 text-destructive" />
        <h1 className="text-xl font-black">وصول غير مصرح به</h1>
        <Button variant="outline" className="font-bold" onClick={() => window.location.href = `/${locale}/admin/login`}>
          العودة للبوابة
        </Button>
      </div>
    );
  ...
  return (
    <div className="grid h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr] overflow-hidden" dir="rtl">
          ...
          <nav className="flex-1 px-2 py-4 space-y-1">
            {navLinks.map(link => (
              <Link
                key={link.href}
                href={link.href}
                className={`flex items-center gap-3 rounded-lg px-3 py-2 transition-all font-bold text-sm ${pathname === link.href
      ...
      <div className="flex flex-col h-full overflow-hidden">
        <header className="flex h-16 items-center justify-between gap-4 border-b bg-card/50 backdrop-blur-md px-6 sticky top-0 z-10 shrink-0">
          <div className="flex items-center gap-4">
            <h1 className="text-lg font-black tracking-widest text-foreground/90 uppercase hidden lg:block">
              العمليات المركزية |{' '}
              <span className="text-primary font-mono text-sm uppercase">
                {isMaster ? 'MASTER' : (profile?.role || 'ADMIN')}
              </span>
            </h1>
          </div>
          <AdminSearchBar />
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuLabel className="text-right">
                مرحباً، {profile?.firstName || 'أيها القائد'}
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={handleSignOut} className="text-destructive font-black cursor-pointer flex justify-between">
                <LogOut className="h-4 w-4" /> إخلاء الموقع
        ...
        <main className="flex-1 p-4 md:p-6 bg-background/30 overflow-y-auto overflow-x-hidden h-full">
          {children}
        </main>
      </div>
    </div>
