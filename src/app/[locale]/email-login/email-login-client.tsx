...
export default function EmailLoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // ✅ تحقق: الـ role المسجّل في الحساب لازم يطابق الـ role الموجود في الـ URL
            const adminRoles = ['admin', 'owner', 'developer', 'operations_manager'];
            const isAdminRole = adminRoles.includes(role);
            if (roleFromUrl && !isAdminRole && role !== roleFromUrl) {
                toast({
                    variant: 'destructive',
                    title: isRTL ? 'حساب غير مطابق' : 'Wrong account type',
                    description: isRTL
                        ? `هذا الحساب مسجّل كـ "${getRoleLabel(role)}"، يرجى الدخول من الصفحة المناسبة`
                        : `This account is registered as "${getRoleLabel(role)}". Please login from the correct page.`,
                });
                setLoading(false);
                return;
            }
