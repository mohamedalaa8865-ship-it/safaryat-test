...
//   return (
//     <div className="min-h-screen bg-background flex flex-col items-center w-full pb-32 overscroll-none" dir="rtl">
//       <div className="w-full max-w-7xl mx-auto sticky top-0 z-50">
//         <TicketHeader tripId={tripId} carrierId={trip.carrierId} tripStatus={trip.status} />
//       </div>
...
//           ) : booking && booking.status === 'Pending-Payment' ? (
//             <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
//               <div className="bg-[#270C13] border-2 border-[#BEAD77] rounded-[2.5rem] p-8 text-center space-y-4">
//                 <div className="text-5xl">✅</div>
//                 <h2 className="text-xl font-black tracking-tight text-white">وافق الناقل على حجزك!</h2>
//                 <p className="text-sm text-muted-foreground font-bold">
//                   أكمل الحجز بدفع العربون الآن لتأكيد مقعدك.
//                 </p>
//                 <div className="bg-muted/30 p-4 rounded-2xl text-right space-y-2">
...
//                 <button
//                   onClick={() => setIsPaymentOpen(true)}
//                   className="w-full h-16 bg-[#BEAD77] hover:bg-[#bead77da] text-black font-black text-xl rounded-3xl transition-all active:scale-95 flex items-center justify-center gap-3"
//                 >
//                   {/* 💳 ادفع العربون الآن */}
//                   التالي
//                 </button>
...
export default function SmartTicketPage() {
  ...
  // [SCR-RATE]: حالة نافذة تقييم الرحلة بعد انتهائها (للمسافر الجاي من الوكيل)
  ...
  // [SCR-RATE]: نفس منطق صفحة history — الرحلة "خلصت" لو الـ status بيقول كذا
  // أو لو وقت الانطلاق + المدة المقدّرة عدّى، حتى لو الناقل لسه ما أكدش
  // نقطة النهاية فعلياً (الحجز لسه Confirmed في القاعدة).
  const isTripDurationEnded = useMemo(() => {
  ...
  const isEnded = trip?.status === 'Completed' || booking?.status === 'Completed' || booking?.status === 'Rated' || isTripDurationEnded;
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center w-full pb-32 overscroll-none" dir="rtl">
          ...
          {booking && booking.status === 'Pending-Carrier-Confirmation' ? (
                ...
                {trip?.excessWeightFee != null && trip.excessWeightFee > 0 && (
                  <div className="flex justify-between items-center bg-orange-950/40 border border-[#AE9E6D] rounded-2xl px-3 py-2.5">
                    ...
                    <span className="text-orange-300 font-mono text-xs font-bold">
                      {trip.excessWeightFee} {trip.currency || booking.currency} / كغ
                    </span>
                  </div>
                )}

                {trip?.conditions && (
                  <div className="bg-yellow-950/40 border border-[#AE9E6D] rounded-2xl px-3 py-2.5 space-y-1 text-right">
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <HeroTicket
                trip={trip}
                booking={booking}
                onRateTrip={() => setIsRatingDialogOpen(true)}
                onCancelBooking={() => setIsCancelOpen(true)}
                onMessageCarrier={() => { }}
                onMessageGroup={() => { }}
              />

              {/* [SCR-RATE]: بعد انتهاء الرحلة — زرار تقييم */}
              {isEnded && (
                <div className="bg-primary/5 p-6 rounded-3xl border-2 border-primary/20 text-center space-y-4 animate-in zoom-in duration-500 shadow-sm mt-4">
                  ...
                  <p className="text-sm text-muted-foreground font-bold">
                    {needsRating
                      ? 'قيّم تجربتك مع الناقل لمساعدة المسافرين الآخرين!'
                      : 'لقد وصلت إلى وجهتك. نتمنى لك رحلة سعيدة!'}
                  </p>
                  {needsRating && (
                    <Button
      ...
      {/* [SCR-RATE]: نافذة تقييم الرحلة بعد انتهائها */}
      <RateTripDialog
