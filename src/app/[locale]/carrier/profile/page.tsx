...
import { Avatar, AvatarFallback } from "@/components/ui/avatar";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Loader2, Save, User, Camera, Car, MapPin, ShieldCheck, Info, Facebook, Instagram, Video } from "lucide-react";
...
export default function CarrierProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDoc(userProfileRef, {
        ...
        ...(formData.tiktokProfile && { tiktokProfile: formData.tiktokProfile }),
        // ✅ إزالة isPartial لو البيانات مكتملة
        isPartial: !formData.vehicleType || !formData.vehicleCapacity,
        photoURL,
        updatedAt: serverTimestamp(),
  ...
  return (
    <div className="container max-w-2xl mx-auto p-4 space-y-6" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* 📱 هوية الثقة الاجتماعية */}
      <Accordion type="single" collapsible className="w-full " defaultValue="social">
        <AccordionItem value="social" className="border rounded-lg bg-[#17070B] border-primary/20">
          <AccordionTrigger className="p-4 font-black text-sm hover:no-underline text-primary">
            <div className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4" />
              هوية الثقة الاجتماعية (جسور التواصل)
            </div>
          </AccordionTrigger>
          <AccordionContent className="p-4 pt-0 space-y-4">
            <div className="bg-[#270C13] p-3 rounded-xl border border-dashed border-primary/20 flex gap-2 items-start mb-4">
              <Info className="h-4 w-4 text-primary shrink-0 mt-0.5" />
              <p className="text-[10px] leading-relaxed text-muted-foreground">
                أضف روابط حساباتك العامة لتعزيز ثقة المسافرين بك. هذا الإجراء يغنيك عن رفع صور المركبة ويسرّع عملية القبول الميداني.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-1">
                <Label className="flex items-center gap-1 text-[10px] font-black">
                  <Facebook className="h-3 w-3 text-blue-600" /> فيسبوك
                </Label>
                <Input
                  placeholder="https://facebook.com/..."
                  className="bg-[#270C13] text-[10px] ltr "
              ...
              <div className="space-y-1">
                <Label className="flex items-center gap-1 text-[10px] font-black">
                  <Instagram className="h-3 w-3 text-pink-600" /> إنستجرام
                </Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="flex items-center gap-1 text-[10px] font-black">
                  <Video className="h-3 w-3 text-foreground" /> تيك توك
                </Label>
                <Input
