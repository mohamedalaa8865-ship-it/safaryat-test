...
//  */
// export const initiateBookingTransfer = onCall({ cors: CORS_ORIGINS }, async (request) => {
//   if (!request.auth) throw new HttpsError("unauthenticated", "AUTH_REQUIRED");
...
//     throw new HttpsError("invalid-argument", "CANNOT_TRANSFER_TO_SELF");
//   }
//   const bookingRef = db.collection("bookings").doc(bookingId);
...
//  */
// export const respondToBookingTransfer = onCall({ cors: CORS_ORIGINS }, async (request) => {
//   if (!request.auth) throw new HttpsError("unauthenticated", "AUTH_REQUIRED");
...
//       throw new HttpsError("permission-denied", "NOT_BOOKING_PASSENGER");
//     }
//     if (accept) {
//       transaction.update(transferRef, {
...
exports.confirmArrivalSovereign = (0, https_1.onCall)({ cors: CORS_ORIGINS }, async (request) => {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        const bookingsSnap = await db
            .collection("bookings")
            .where("tripId", "==", tripId)
            .where("status", "in", ["Confirmed", "Pending-Payment", "Pending-Payment-Verification"])
            .get();
        bookingsSnap.docs.forEach((bookingDoc) => {
            transaction.update(bookingDoc.ref, {
                status: "Completed",
